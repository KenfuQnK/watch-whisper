...
const mapFromDb = (row: any): MediaItem => {
  ...
  return {
    id: row.id,
    title: row.title,
    originalTitle: row.original_title, // New field
    type: row.type,
    ...
    isEnriched: row.is_enriched || false, // New field
    seasons: row.seasons,
...
const mapToDb = (item: MediaItem) => {
  ...
  return {
    id: item.id,
    title: item.title,
    original_title: item.originalTitle, // New field
    type: item.type,
    ...
    is_enriched: item.isEnriched, // New field
...
export const updateMediaItem = async (id: string, changes: Partial<MediaItem>) => {
  ...
  if (changes.originalTitle !== undefined) dbChanges.original_title = changes.originalTitle;
  ...
  if (changes.isEnriched !== undefined) dbChanges.is_enriched = changes.isEnriched;
